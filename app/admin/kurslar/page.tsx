    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="font-display text-3xl font-bold text-metin">Kurslar</h1>
        </div>
        <Link
            ...
            <tbody>
              {kurslar.map((k) => (
                <tr key={k.id} className="border-b border-cizgi last:border-0 hover:bg-zemin/60 transition-colors">
                  ...
                  <td className="px-5 py-3 text-metin font-medium">{k.baslik}</td>
                  <td className="px-5 py-3 text-metin/60">{k.seviye}</td>
                  <td className="px-5 py-3 text-metin/60">{k._count.lessons}</td>
                  <td className="px-5 py-3 text-right space-x-4">
                    <Link href={`/admin/kurslar/${k.id}/duzenle`} className="text-vurgu hover:text-vurgu-dark">
                      Düzenle
                    </Link>
