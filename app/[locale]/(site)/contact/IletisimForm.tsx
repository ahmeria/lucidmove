...
export default function IletisimForm() {
  ...
  if (durum === "basarili") {
    return (
      <div className="border border-vurgu/40 bg-vurgu/10 rounded-2xl p-8 font-body">
        <p className="text-metin font-medium">{t("iletildi")}</p>
        <p className="text-metin/70 text-sm mt-2">{t("enKisaSurede")}</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 font-body">
      ...
      <div>
        <label htmlFor="mesaj" className="block text-sm text-metin/70 mb-1.5">
          {t("mesajiniz")}
        </label>
        <textarea
          id="mesaj"
      ...
      {durum === "hata" && <p className="text-sm text-red-700">{hataMesaji}</p>}
      ...
      >
        {durum === "gonderiliyor" ? t("gonderiliyor") : t("mesajiGonder")}
      </button>
    </form>
  );
}
