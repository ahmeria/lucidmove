...
export default async function GizlilikPolitikasi() {
  const [t, ayarlar, locale] = await Promise.all([getTranslations("privacy"), getSiteSettings(), getLocale()]);
  const sonGuncelleme = new Intl.DateTimeFormat(intlEtiketi(locale as AppLocale), { dateStyle: "long" }).format(
    new Date()
  );

  const bolumler = [1, 2, 3, 4, 5, 6, 7].map((n) => ({
    baslik: t(`s${n}Baslik`),
    metin: t(`s${n}Metin`, { eposta: ayarlar.iletisimEmail }),
  }));
