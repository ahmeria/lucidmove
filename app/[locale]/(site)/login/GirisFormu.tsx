...
export default function GirisFormu() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (sonuc?.error) {
      setHata(t("epostaSifreHatali"));
      setYukleniyor(false);
      return;
    }

    // returnTo param'ı açıkça verilmemişse role'e göre karar ver (admin -> /admin,
    // üye -> /account, seçili locale korunarak). Session cookie'sinin taze
    // halini garantilemek ve RSC/prefetch cache'inin eski (oturumsuz) durumu
    // göstermesini önlemek için client-side router yerine tam sayfa (hard)
    // navigasyon kullanıyoruz.
    const session = await getSession();
    const varsayilanYol = session?.user?.role === "ADMIN" ? "/admin" : "/account";
    const varsayilan =
      varsayilanYol === "/admin" ? varsayilanYol : getPathname({ href: varsayilanYol, locale });
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 py-16">
      ...
      <div className="relative w-full max-w-md animate-riseIn">
        ...
        <div className="bg-kart rounded-[2rem] shadow-organik-hover p-8 sm:p-10">
          <p className="font-mono text-xs tracking-[0.3em] uppercase text-vurgu-dark mb-3">{t("girisYap")}</p>
          <h1 className="font-display text-3xl font-bold text-metin leading-tight">{t("hosGeldiniz")}</h1>
          <p className="font-body text-sm text-metin/60 mt-3">{t("girisAltMetin")}</p>
