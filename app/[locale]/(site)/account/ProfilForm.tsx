...
export default function ProfilForm({
  ...
  async function fotoKirpmaTamamlandi(kirpilmisDosya: File) {
    ...
    try {
      ...
      if (!res.ok) {
        setHata(veri.hata || t("fotografYuklenemedi"));
        return;
      }
      setProfilFotoUrl(veri.url);
      router.refresh();
    } catch {
      setHata(t("fotografYuklenemediBaglanti"));
    } finally {
    ...
    <div className="space-y-6 font-body">
      <div className="flex items-center gap-4">
        ...
        <div>
          <label className="inline-block cursor-pointer text-sm text-vurgu hover:text-vurgu-dark">
            {fotoYukleniyor ? t("yukleniyor") : t("fotografDegistir")}
            <input
              type="file"
          ...
          <p className="text-xs text-metin/45 mt-0.5">{t("fotografFormatBilgisi")}</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm text-metin/70 mb-1.5">{t("adSoyad")}</label>
          <input
            value={ad}
        ...
        <div>
          <label className="block text-sm text-metin/70 mb-1.5">{t("eposta")}</label>
          <input
            value={email}
            disabled
        ...
        <div>
          <label className="block text-sm text-metin/70 mb-1.5">{t("telefon")}</label>
          <input
            value={telefon}
        ...
        {kaydedildi && <p className="text-sm text-vurgu-dark">{t("kaydedildi")}</p>}
        ...
        >
          {gonderiliyor ? t("kaydediliyor") : t("kaydet")}
        </button>
      </form>
      ...
      {seciliFoto && (
        <GorselKirpici
          ...
          oran={1}
          daire
          baslik={t("profilFotografiniziYerlestirin")}
